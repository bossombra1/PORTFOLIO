...
const Projects = ref([
    {
        ...
        webURL: '#' // <-- Ajouté pour éviter l'erreur undefined
    },
    {
        id: 2,
        category: 'Mobile App',
    ...
    },
    {
        ...
        image: 'taches.jpg',
        title: 'TaskFlow',
        description: 'Système de gestion de tâches collaborative. API REST sécurisée avec JWT, frontend Angular moderne, infrastructure Dockerisée.',
    ...
    },
    {
        ...
        webURL: 'https://bossombra1.github.io/PORTFOLIO/' 
    },
]);
...
// Fonction essentielle pour injecter dynamiquement les images au build de Vite
            ...
            <ul class="px-4 sm:py-16 xl:pr-16 grid grid-cols-1 gap-6 pt-10 sm:grid-cols-2 md:gap-10 md:pt-12 lg:grid-cols-3"
                ...
                <div v-for="project in filteredProjects" :key="project.id" class="flex flex-col h-full">
                    ...
                    <div class="h-52 md:h-[24rem] rounded-t-xl relative group"
                         :style="{ backgroundImage: 'url(' + getImgUrl(project.image) + ')', backgroundSize: 'cover', backgroundPosition: 'center' }">                    
                    ...
                    <div class="text-white rounded-b-xl bg-[#111a3e] shadow-lg border border-[#1f1641] py-6 px-4 flex-grow flex flex-col justify-between">
                        <div>
                            <h3 class="text-lg font-semibold uppercase lg:text-xl">{{ project.title }}</h3>
                            <p class="text-[#ADB7BE] text-sm mt-2">{{ project.description }}</p>
                        </div>
                        <div class="flex flex-wrap p-2.5 mt-4">
                            <div v-for="technology in project.technologies" :key="technology" class="text-center ml-1 mt-1 rounded-3xl bg-[#111827]"
                                ...
                                <p class="px-3 py-1 text-xs">{{ technology }}</p>
                            </div>
                        </div>
                    </div>
